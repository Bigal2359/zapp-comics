...
$an_plugin_settings = array(
	...
	'request_item_endpoint' => '//www.auctionnudge.com/feed/item/js/',
	'request_profile_endpoint' => '//www.auctionnudge.com/feed/profile/js/',
	'request_feedback_endpoint' => '//www.auctionnudge.com/feed/feedback/js/',
	'request_ad_endpoint' => '//www.auctionnudge.com/feed/ad/iframe/',
	'username_bad' => array('.', "\$", '!', '*'),
		...
		if($p_value !== '') {
			...
			switch($p_key) {
				...
				case 'SellerID':
					...
					break;
				case 'theme':
					//Overview theme for profile tool
					if($tool == 'PROFILE' && $p_value == 'overview') {
						$profile_is_framed = true;
					}
					break;
			}
	...
	switch($tool) {
		case 'PROFILE' :
			...
			//Iframe
				...
				$url = str_replace('feed/profile/js', 'feed/profile/iframe', $an_plugin_settings['request_profile_endpoint']);
				$src = $url . $src;
				$profile_framed_width = '250px';				
				$profile_framed_height = '288px';				

				$out = '<iframe width="' . $profile_framed_width . '" height="' . $profile_framed_height . '" style="border:none" frameborder="0" src="' . $src . '"></iframe>';												
			}
	...
	switch($tool) {
		case 'PROFILE' :
			...
			foreach($an_plugin_settings['profile_parameters'] as $field) {
				if(array_key_exists($field['name'], $post_meta)) {
					if(array_key_exists($field['name'], $post_meta)) {
						$request_parameters[$field['name']] = $post_meta[$field['name']][0];					
					}					
				}
			}
...
function an_create_custom_field_form() {
	...
	if(an_get_option('an_ads_disable') == false) {
		...
		echo '	<div class="an-custom-field-help">' . "\n";
		echo '		<p><b>The Your eBay Ads tool is no longer recommended due to very poor conversion rates.</b> Instead we recommend the <b>Your eBay Listings</b> tool. More information <a target="_blank" href="https://www.auctionnudge.com/help#faq-ads-retired">here</a>.</b></p>' . "\n";
	...
	switch($field['type']) {
		...
		default :
			...
			if($value = htmlspecialchars($set_value)) {
				$out .= ' value="' . $value . '"';
			//Setting?
			} elseif(strpos($field['name'], 'SellerID') && array_key_exists('an_ebay_user', $options) && $options['an_ebay_user']) {
				$out .= ' value="' . $options['an_ebay_user'] . '"';				
			} elseif(strpos($field['name'], 'UserID') && array_key_exists('an_ebay_user', $options) && $options['an_ebay_user']) {
				$out .= ' value="' . $options['an_ebay_user'] . '"';				
			//Do we have a default?
...
function an_options_page() {
	...
	echo '		<p>I highly recommend watching the <a target="_blank" href="https://www.youtube.com/watch?v=smamgdwCD74">video tutorial</a> on how to use the plugin.</p>' . "\n";	
	...
	switch($_GET['tab']) {
		case 'theme' :
			...
			echo '<p>In <em>most</em> cases you will not need to use these options. If you are unsure how to use the plugin, I recommend checking out this quick <a target="_blank" href="https://www.youtube.com/watch?v=smamgdwCD74">Video Tutorial</a>.</p>' . "\n";
...
function an_items_setting() {
	...
	echo '<textarea id="an_items_code_snippet" name="an_options[an_items_code]" rows="6" columns="30" style="font-family:courier;width:400px">' . $an_items_code  . '</textarea>' . "\n";
}
...
function an_profile_setting() {
	...
	echo '<textarea id="an_profile_code_snippet" name="an_options[an_profile_code]" rows="6" columns="30" style="font-family:courier;width:400px">' . $an_profile_code . '</textarea>' . "\n";
}
...
function an_feedback_setting() {
	...
	echo '<textarea id="an_feedback_code_snippet" name="an_options[an_feedback_code]" rows="6" columns="30" style="font-family:courier;width:400px">' . $an_feedback_code . '</textarea>' . "\n";
}
...
function an_ebay_user_text() {
	echo '<p>Your eBay User ID is the username that is associated to your eBay account and appears on your listings.</p>' . "\n";
	echo '<p>Entering a default eBay user ID here will save you from re-entering it on each page, post or widget where you want to use Auction Nudge.</p>' . "\n";
 	echo '<p>Should you change eBay username, you can also use this option to update every Auction Nudge instance with the new setting.</p>';	
}
...
function an_ebay_user_setting() {
	...
	echo '<small>Update every instance</small>' . "\n";	
}
...
function an_css_setting() {
	...
	echo '<textarea id="an_css_rules" name="an_options[an_css_rules]" rows="6" columns="30" style="font-family:courier;width:400px">' . $an_css_rules . '</textarea>' . "\n";		
}
